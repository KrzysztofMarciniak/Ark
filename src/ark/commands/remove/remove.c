...
int ark_remove_package(const struct ark_package* package) {
        ...
        if (snprintf(script_path, sizeof(script_path), "%s/.ark-remove.sh",
                     remove_directory) >= (int)sizeof(script_path)) {
...
int ark_command_remove(int argc, char** argv) {
        const char* home;
        ...
        if (argc < 1) {
                fprintf(stderr, "ark: remove: missing package\n");
        ...
        printf("==> Searching recipes for %s\n", argv[0]);

        if (ark_find_package(recipes_root, argv[0], &package) != 0) {
                fprintf(stderr, "ark: package not found: %s\n", argv[0]);
...
ARK_COMMAND("remove", "ark remove <package>", "Remove a package",
            ark_command_remove);
