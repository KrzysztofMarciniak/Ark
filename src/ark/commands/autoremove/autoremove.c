...
static int
name_index(char **names, size_t count, const char *name)
{
    size_t i;
...
)
{
    ...
    if (ark_installed_list(installed_root, &names) != 0) {
        fprintf(
            stderr,
            "ark: could not read installed packages\n"
        );
    ...
    for (i = 0; i < count; i++) {
        ...
        is_explicit[i] = ark_installed_is_explicit(installed_root, names[i]);

        if (ark_installed_get_depends(
                installed_root,
                names[i],
                depends_storage,
                sizeof(depends_storage),
                deps,
    ...
    for (i = 0; i < count; i++) {
        ...
        if (ark_find_package(
            ...
            fprintf(
                ...
                "forgetting install record without running remove()\n",
                names[i]
            );
...
ark_command_autoremove(int argc, char **argv)
{
     ...
     */
    for (;;) {
        result = autoremove_pass(recipes_root, installed_root, assume_yes);
