...
)
{
    ...
    for (i = 0; i < state->package_count; i++) {
        if (strcmp(state->packages[i], name) == 0)
            return 1;
    }
...
)
{
    ...
    if (state->package_count == state->package_capacity) {
        ...
        capacity = state->package_capacity
            ? state->package_capacity * 2
            : 16;
...
)
{
    ...
    if (depth > ARK_MAX_DEPENDENCY_DEPTH) {
        fprintf(
            stderr,
            "ark: dependency tree too deep at %s\n",
            name
        );
     ...
     */
    if (dependency_mark(state, package.name) != 0)
        return -1;
