...
static int find_highest_version(const char* package_dir, char* version_out,
        ...
        while ((entry = readdir(dir)) != NULL) {
                ...
                if (!is_directory(candidate_path)) continue;

                if (snprintf(recipe_path, sizeof(recipe_path), "%s/recipe.sh",
                             candidate_path) >= (int)sizeof(recipe_path))
                        continue;

                if (!ark_is_regular_file(recipe_path)) continue;

                if (!found || version_compare(entry->d_name, version_out) > 0) {
                        if (strlen(entry->d_name) >= version_out_size) continue;
 ...
 */
static int read_depends(const char* recipe_path, char* buffer,
                        size_t buffer_size) {
        ...
        if (snprintf(command, sizeof(command),
                     ...
                     recipe_path) >= (int)sizeof(command))
                return -1;
...
static void split_depends(struct ark_package* package) {
        char* token;
        char* saveptr;
        size_t i;
...
int ark_find_package(const char* recipes_root, const char* name,
        ...
        if (strlen(name) >= sizeof(package->name)) {
                fprintf(stderr, "ark: package name too long: %s\n", name);
        ...
        if (dir == NULL) {
                fprintf(stderr, "ark: cannot open recipes root: %s\n",
                        recipes_root);
        ...
        while ((entry = readdir(dir)) != NULL) {
                ...
                if (snprintf(repo_path, sizeof(repo_path), "%s/%s",
                             recipes_root,
                             entry->d_name) >= (int)sizeof(repo_path))
                        continue;

                if (!is_directory(repo_path)) continue;

                if (snprintf(package_dir, sizeof(package_dir), "%s/%s",
                             repo_path, name) >= (int)sizeof(package_dir))
                        continue;
                ...
                if (find_highest_version(package_dir, version,
                                         sizeof(version)) != 0)
                        continue;

                if (!found || version_compare(version, best_version) > 0) {
                        if (snprintf(best_recipe_path, sizeof(best_recipe_path),
                                     "%s/%s/recipe.sh", package_dir,
                                     version) >= (int)sizeof(best_recipe_path))
                                continue;
        ...
        if (read_depends(package->recipe_path, package->depends_storage,
                         sizeof(package->depends_storage)) != 0) {
                fprintf(stderr, "ark: failed to read ARK_DEPENDS for %s\n",
                        name);
