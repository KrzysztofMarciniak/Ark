...
static int
write_field(const char *path, const char *content)
{
    FILE *file;
...
static int
read_field(const char *path, char *buffer, size_t buffer_size)
{
    FILE *file;
    size_t length;
...
)
{
    ...
    if (package_dir_path(installed_root, name, package_dir, sizeof(package_dir)) != 0)
        return 0;
...
)
{
    ...
    if (field_path(package_dir, "explicit", explicit_path, sizeof(explicit_path)) != 0)
        return 0;
...
)
{
    ...
    if (package_dir_path(installed_root, package->name, package_dir, sizeof(package_dir)) != 0)
        return -1;

    already_explicit = ark_installed_is_explicit(installed_root, package->name);

    if (mkdir_parents(installed_root) != 0)
        return -1;

    if (mkdir(package_dir, 0700) != 0 && errno != EEXIST)
        return -1;

    if (field_path(package_dir, "version", version_path, sizeof(version_path)) != 0)
        return -1;

    if (write_field(version_path, package->version) != 0)
        return -1;
    ...
    for (i = 0; package->depends[i] != NULL; i++) {
        ...
        written = snprintf(
            depends_line + offset,
            sizeof(depends_line) - offset,
            "%s%s",
            offset > 0 ? " " : "",
            package->depends[i]
        );

        if (written < 0 || (size_t)written >= sizeof(depends_line) - offset)
            break;

        offset += (size_t)written;
    }

    if (field_path(package_dir, "depends", depends_path, sizeof(depends_path)) != 0)
        return -1;

    if (write_field(depends_path, depends_line) != 0)
        return -1;
    ...
    if (explicit || already_explicit) {
        if (write_field(explicit_path, "") != 0)
            return -1;
    }
...
)
{
    ...
    if (!ark_installed_exists(installed_root, name))
        return -1;
...
)
{
    ...
    field_path(package_dir, "explicit", explicit_path, sizeof(explicit_path));

    unlink(version_path);
    unlink(depends_path);
    unlink(explicit_path);

    if (rmdir(package_dir) != 0 && errno != ENOTEMPTY)
        return -1;
...
)
{
    ...
    if (read_field(depends_path, depends_storage, depends_storage_size) != 0) {
        depends_storage[0] = '\0';
    }
...
)
{
    ...
    *names_out = NULL;
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (package_dir_path(installed_root, entry->d_name, package_dir, sizeof(package_dir)) != 0)
            continue;
